...
const CreateProfile = ({ route, navigation, profile }) => {
  ...
  const [selectedYLocation, setSelectedYLocation] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://192.168.25.60:3000/collections", {
  ...
  const checkValidation = () => {
    if (!profile) return "Profile information is required.";
    ...
    if (!selectedYLocation) return "Please select a location.";
    if (!selectedCity) return "City is required.";
    if (!address) return "Address is required.";
    if (!personalInformation) return "Personal information is required.";
    if (!emergencyInformation) return "Emergency information is required.";
        ...
        <ProgressSteps activeStep={activeStep}>
          ...
          >
            ...
            <SectionWithHeader title="Your Location" onEditPress={onOpen[4]}>
              <View>
                <Text>
                  {locations?.find((cl) => cl?._id == selectedYLocation)?.name}
                </Text>
              </View>
            </SectionWithHeader>
            <SectionWithHeader
