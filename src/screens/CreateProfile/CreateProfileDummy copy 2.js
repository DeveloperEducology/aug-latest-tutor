...
const ProfileInfoSection = ({ title, information, handleChange, editableField, handleEdit }) => (
...
const CreateProfile = ({ navigation }) => {
  ...
  // Selection Hooks
  const [selectedCategories, handleCategorySelection, setSelectedCategories] = useSelection();
  const [selectedClasses, handleClassSelection, setSelectedClasses] = useSelection();
  const [selectedSubjects, handleSubjectSelection, setSelectedSubjects] = useSelection();
  const [preferredLocations, handleLocationSelection, setPreferredLocations] = useSelection();
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          Alert.alert("Error", errorData.message || "Failed to fetch collections");
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (selectedCity) {
      const filtered = locationsData.filter((loc) => loc.cityId === selectedCity);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`http://192.168.204.30:3000/create-profile`, {
        method: "POST",
        headers: {
              ...
              >
                <View style={styles.chipsContainer}>
                  {selectedClasses.map((id) => {
                    const classItem = classesData.find((item) => item._id === id);
                    return (
              ...
              >
                <View style={styles.chipsContainer}>
                  {selectedSubjects.map((id) => {
                    const subject = subjectsData.find((item) => item._id === id);
                    return (
                      <View key={id} style={styles.chip}>
                  ...
                  {selectedCity && (
                    <View style={styles.chip}>
                      <Text style={styles.chipText}>
                        {citiesData.find((city) => city._id === selectedCity)?.name}
                      </Text>
                    </View>
                  )}
                </View>
              ...
              >
                <View style={styles.chipsContainer}>
                  {preferredLocations.map((id) => {
                    const location = locationsData.find((item) => item._id === id);
                    return (
                      <View key={id} style={styles.chip}>
